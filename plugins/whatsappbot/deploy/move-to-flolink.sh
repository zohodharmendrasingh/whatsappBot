...
# Before running: in Cloudflare point flolink.ai, www.flolink.ai and app.flolink.ai (A records, DNS only / grey cloud)
# to this server, and keep flochat.flolink.ai pointing here too.
set -euo pipefail
EMAIL=${EMAIL:-info@msoftdynamic.com}
CONF=/etc/flochat/flochat.properties
for h in flolink.ai www.flolink.ai app.flolink.ai; do
...
certbot --nginx --non-interactive --agree-tos -m "$EMAIL" --expand \
  -d flolink.ai -d www.flolink.ai -d app.flolink.ai -d flochat.flolink.ai --redirect
