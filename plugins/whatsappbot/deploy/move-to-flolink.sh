...
cp --update=none /etc/nginx/sites-available/flochat /etc/nginx/sites-available/flochat.bak-$(date +%Y%m%d) || true
...
mkdir -p "$(dirname "$CONF")"; touch "$CONF"; cp "$CONF" "$CONF.bak-$(date +%Y%m%d)"
